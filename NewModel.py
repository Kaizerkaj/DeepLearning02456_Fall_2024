...
epochs = int(sys.argv[1]) # int(sys.argv[1]) # 1980 epoch for DDPM paper 

assert sys.argv[2] == "MNIST" or sys.argv[2] == "CIFAR"

dataset = sys.argv[2] # "MNIST" or "CIFAR"
...
def main():
    ...
    model, iVal = loadModel(diffusion, train_loader)
...
class UNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 down_ch: list = [x // 2 for x in [64, 128, 256, 512]],  # Channels for DownC blocks
                 mid_ch: list = [x // 2 for x in [512, 512, 256]],  # Channels for MidC blocks
                 up_ch: list[int] = [x // 2 for x in [512, 256, 128, 64]],  # Channels for UpC blocks
                 down_sample: list[bool] = [True, True, False],  # Downsample flag per DownC block
                 t_emb_dim: int = 512,  # Time embedding dimension
                 num_downc_layers: int = 3,  # Layers in each DownC block
                 num_midc_layers: int = 3,  # Layers in each MidC block
                 num_upc_layers: int = 3,  # Layers in each UpC block
...
def train_model(model, data_loader, diffusion, optimizer, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        for x0, _ in data_loader:
            ...
            t = torch.randint(1, diffusion.timesteps + 1, (x0.size(0),), device = device)  # Random timesteps
        ...
        print(f"Epoch {epoch + 1} / {epochs} done. {elapsed} seconds elapsed.")
    total_elapsed = timeit.default_timer() - total_time_start
    totalSteps = len(train_dataset) * epochs / batch_size
    print(f"\nTraining speed: {(totalSteps / total_elapsed):.1f} steps/sec")
    # step = (50000 * epoch) / batchsize 
    # (batchsize * step) / 50000 = epoch
...
def loadModel(diffusion, train_loader):
    ...
    if dataset == "CIFAR":
        modelName = "CIFAR"
    while f"{modelName}{i}E{epochs}T{timesteps}.pth" in files:
        i += 1
    newName = f"{modelName}{i}E{epochs}T{timesteps}"
    print(f"\nNew model: '{newName}.pth' created.")
