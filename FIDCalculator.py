...
batch_size = 128
num_samples = 50000 # 50,000 used in DDPM paper
channels = 3
imgSize = (32, 32)
...
def main():
    ...
    model.eval()
    images = reverse_process(model, diffusion, device, num_steps=1000, num_samples=num_samples, batch_size=256)

    generated_images = torch.tensor(np.array(images)).to(device)

    # Calculate FID
    fid_score = compute_fid(test_loader, generated_images, device)
    enablePrint()
    print(f"{sys.argv[1]}: {fid_score}", end = "")
...
def reverse_process(model, diffusion, device, num_steps=1000, num_samples=50000, batch_size=256):
    all_images = []
    ...
    for i in range(0, num_samples, batch_size):
        ...
        with torch.no_grad():
            x_t = torch.randn((current_batch_size, channels, imgSize[0], imgSize[1]), device=device)  # Noise sample
            timesteps = torch.linspace(num_steps, 1, current_batch_size, dtype=torch.long)

            for t in range(num_steps, 0, -1):
                t_tensor = torch.full((current_batch_size,), t, device=device, dtype=torch.long)

                epsilon_theta = model(x_t, t_tensor)  # Predict noise using trained model
            ...
            # Append final images to all_images list
            all_images.append(x_t.detach().cpu().numpy())
...
def loadModel(diffusion, train_loader):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # use cpu
    epochVal = sys.argv[1]
    model = torch.load(path + f"/{epochVal}/CIFAR10.pth", map_location = device).to(device)
